...
export async function GET() {
  ...
  const oauthUrl = process.env.SHOPIFY_CUSTOMER_ACCOUNT_API_URL && process.env.SHOPIFY_CUSTOMER_ACCOUNT_API_CLIENT_ID
    ? `${process.env.SHOPIFY_CUSTOMER_ACCOUNT_API_URL}/oauth/authorize?client_id=${process.env.SHOPIFY_CUSTOMER_ACCOUNT_API_CLIENT_ID}&response_type=code&scope=openid%20email%20profile%20https://api.shopify.com/auth/customer.graphql&redirect_uri=${encodeURIComponent(redirectUri)}&state=test`
